...
/*---- start: c:/daily/rwsdk/plugin/ptank/xbox/ptankplatform.h----*/

/*---- start: c:/daily/rwsdk/plugin/ptank/xbox/ptankxbox.h----*/

/**
 * \defgroup rpptankxbox Xbox
 * \ingroup rpptank
 *
...
/**
 * \ingroup rpptankxbox
 * \par Xbox specific data flags
 *
 * \li rpPTANKXBOXOPTIMISEDPIPES  selects the Xbox optimised pipelines. At the 
 * moment, these pipelines use PointSprites, which don't allow use of all the 
 * PTank flags. Only the following flags are supported when using the Xbox 
 * optimised pipes
 * rpPTANKDFLAGPOSITION
 * rpPTANKDFLAGCOLOR
 * The texture coordinates are generated by the hardware and can't be specified.
 * In order to access the data when using the optimised pipes, you must use the 
 * following functions
 * \ref RpPTankAtomicXboxLock
 * \ref RpPTankAtomicXboxUnlock
 * \ref RpPTankAtomicXboxUpdatePosition
 * \ref RpPTankAtomicXboxUpdateColor
 * 
 */
enum RpPTankXboxFlags
{
    rpPTANKXBOXOPTIMISEDPIPES = 0x00000001,

    RPPTANKXBOXFLAGSFORCEENUMSIZEINT = RWFORCEENUMSIZEINT
};

typedef enum RpPTankXboxFlags RpPTankXboxFlags;
...
extern void _rwXbAtomicDefaultLightingCallback(void* object);

#ifdef    __cplusplus
}
#endif /* __cplusplus */

/*---- end: c:/daily/rwsdk/plugin/ptank/xbox/ptankxbox.h----*/
/*---- start: c:/daily/rwsdk/plugin/ptank/xbox/ptankxboxopt.h----*/

/**
 * \ingroup rpptankxbox
 * \ref RpPTankAtomicXboxLock
 * Allows the application to the data contained in the PTank atomic
 *
 * \param atomic a pointer to the \ref RpAtomic to be updated
 */
void
RpPTankAtomicXboxLock( RpAtomic* atomic );

/**
 * \ingroup rpptankxbox
 * \ref RpPTankAtomicXboxUnlock
 * Specifies that the application has finished updating the atomic
 *
 * \param atomic a pointer to the \ref RpAtomic to be unlocked
 */
void 
RpPTankAtomicXboxUnlock( RpAtomic* atomic );

/**
 * \ingroup rpptankxbox
 * \ref RpPTankAtomicXboxUpdatePosition
 * Updates the position of a particular particle
 *
 * \param atomic a pointer to the \ref RpAtomic
 * \param pos the new position
 * \param partIndex the index of the particle to be updated
 */
void 
RpPTankAtomicXboxUpdatePosition( RpAtomic* atomic, RwV3d* pos, RwUInt32 partIndex);

/**
 * \ingroup rpptankxbox
 * \ref RpPTankAtomicXboxUpdateColor
 * Updates the color of a particular particle
 *
 * \param atomic a pointer to the \ref RpAtomic
 ...
 */
void 
RpPTankAtomicXboxUpdateColor( RpAtomic* atomic, RwRGBA* color, RwUInt32 partIndex);

/*---- end: c:/daily/rwsdk/plugin/ptank/xbox/ptankxboxopt.h----*/

/*---- end: c:/daily/rwsdk/plugin/ptank/xbox/ptankplatform.h----*/
